...
- [~] **角色特性（百相）实装**：`CharaterInfo` 使用 `Trait` 字段；游侠 `MoveAllyGrantMorale`＝每当周周通过卡牌成功移动一名友方角色时，使其获得1层士气。法师 `DoubleExecution` 与战士 `FirstPushFree` 已有对应逻辑；通用的可组合百相框架仍待做。
  - [x] `Morale` Buff：永久、最多3层；移动自己、单体队友和整排友方时，被成功移动的角色分别获得1层。
  - [x] 士气消费：所有角色使用伤害牌时消耗全部士气；每层使该牌每一段基础伤害+2。群体牌的每个目标、以及多段牌的每一段都使用加成后的基础伤害；状态与移动触发伤害不消费。
...
- [ ] 待测：法师执行是否可被敌人打断；过载「越用越贵」是否需要；移动/士气/飞刀/闪避数值。
