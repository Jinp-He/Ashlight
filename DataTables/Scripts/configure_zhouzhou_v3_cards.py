...
CARDS = {
    ...
    "Zhouzhou005": card("挽袖同行", "[移动]一名其他友方角色；使其获得{D}点护甲。", "MovePositionEffect,N,Toggle;DefenseEffect,D,5,false", "SingleAlly", "基础", 1, reward="FALSE"),
    ...
    "Zhouzhou008": card("预知一击", "对当前公共回合的所有敌方角色造成{A}点伤害。", "AttackCurrentRoundEffect,A,4", "AllEnemy", "基础", 0, reward="FALSE"),
    ...
    "Zhouzhou012": card("回锋", "造成{A}点伤害；若这张牌消耗了[士气]，随后[移动]。", "AttackEffect,A,5,false", "SingleEnemy", "普通", 0),
    "Zhouzhou013": card("行云", "[移动]；若移动后拥有3层[士气]，抽1张牌。", None, "Self", "普通", 1),
    "Zhouzhou014": card("折返", "连续[移动]2次；第二次移动成功后，获得1层[闪避]。", None, "Self", "稀有", 1),
    ...
    "Zhouzhou018": card("飒沓流星", "这个回合中，接下来每当你成功移动，抽1张牌，最多触发2次；随后[移动]，并将1张[飞刀]加入手牌。", None, "Self", "稀有", 1),
    "Zhouzhou019": card("一鼓作气", "消耗自身所有[士气]；所有友方角色获得5点护甲，每消耗1层，额外获得2点护甲。", None, "AllAlly", "稀有", 2),
    "Zhouzhou020": card("旧步重寻", "获得这个回合中最近打出的一张移动主卡的0费[闪回]副本。", None, "Self", "稀有", 1),
    "Zhouzhou021": card("千里不留行", "这个回合中，你的移动牌费用变为0；将3张[移步]加入手牌。", None, "Self", "史诗", 2),
    "Zhouzhou022": card("十步杀一人", "造成8点伤害；这个回合中，你每成功移动过1次，重复1次，最多重复2次。", None, "SingleEnemy", "史诗", 0),
    "Zhouzhou023": card("满场飞花", "[过载]1。依次[移动]至多3名不同的友方角色；被移动的角色各获得1层[闪避]。", "OverloadEffect,F,1", "AllAlly", "史诗", 2),
}
    ...
    for row in range(4, sheet.max_row + 1):
        if sheet.cell(row, headers["Character"]).value == "Zhouzhou":
            sheet.cell(row, headers["BaseDeck"]).value = (
                "Zhouzhou001,Zhouzhou001,Zhouzhou002,Zhouzhou002,Zhouzhou003,"
                "Zhouzhou004,Zhouzhou005,Zhouzhou006,Zhouzhou007,Zhouzhou008"
            )
            break
    else:
