...
现行设计基准见 `docs/规则书_角色卡牌设计.md`（v2，最高优先），辅以 `docs/职业设计_三职业与公用系统.md`、`docs/天气系统设计_v1.md`、`docs/敌人压迫感设计_v1.md`。旧方案已归档至 `docs/archive/`，勿作依据。文档导航见 `docs/README.md`。

- 行动顺序由公共回合时钟驱动（Speed = 每 N 回合动一次，数值越大越慢；法4/战3/游2）。
- 玩家侧是能量 + 每角色独立手牌/抽弃堆；卡牌分即时牌与执行牌（执行牌 = ATB 虚拟单位真延迟）。
- 敌人是单轨原子回合（宣告 → 等 ExecutingCost → 结算），意图可被推迟/打断。
- 两区站位（前排/后排）+ 分区索敌；天气（雷暴）是 ATB 虚拟单位第三方阵营。
