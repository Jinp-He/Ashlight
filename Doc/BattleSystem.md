...
> ⚠️ **已过时（2026-07-09 标注）**：本文描述的「0–100 段位 ATB + 全场暂停 + 意图轴/执行轴双轴敌人」是重构前的旧系统，**勿作为设计或实现依据**。现行基准：[规则书_角色卡牌设计](../docs/规则书_角色卡牌设计.md)（公共回合离散调度）、[职业设计_三职业与公用系统](../docs/职业设计_三职业与公用系统.md)、[天气系统设计_v1](../docs/天气系统设计_v1.md)。
> 主要过时点：① 0–100 段位行动条已改为**公共回合时钟**（Speed=行动间隔，数值越大越慢，本文的 Speed 语义相反）；② `ActionBarResolver.cs` 已删除；③ 过载不再是 5/12/25 段位负债，而是**下次行动 +N 回合**；④ 敌人已改单轨原子回合（无双轴）；⑤ 玩家执行牌现为回合结束立即结算。
